...
int main()
{
    try
    {
        ...
        //Фильтрация: любой байт 46 (decltype)
        using ip_item = decltype(ip_pool)::value_type;
        for (const ip_item &ip : ip_pool)
        {
            if (std::any_of(ip.begin(), ip.end(), [](int byte)
                            { return byte == 46; }))
            {
                print_ip(ip);
            }
...
// g++ -std=c++11 ip_filter.cpp -o ip_filter.exe
// Get-Content ip_filter.tsv | ./ip_filter.exe